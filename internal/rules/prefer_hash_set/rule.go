...
var constructorMessage = rule.RuleMessage{Id: "prefer-hash-set", Description: "Avoid constructing a built-in Set.", Help: "Use Effect's HashSet instead — for example HashSet.fromIterable([1, 2, 3]) or HashSet.empty(). HashSet uses Equal and Hash with structural equality by default. For reference-identity object members, retain one Equal.byReference wrapper and use that same wrapper for every insertion and membership check. Each call creates a new wrapper, so wrapping the raw object again does not preserve native-identity membership. If a public contract must accept raw object identities, moving to HashSet changes that contract; do not use Equal.byReferenceUnsafe as a general replacement. Constructing a Set is exempt when it is passed directly, or through a variable, to a resolved call declared outside the current source file; that includes first-party modules, not only third-party APIs."}
var typeHint = "Use HashSet.HashSet<T> from Effect instead. HashSet uses Equal and Hash with structural equality by default. For reference-identity object members, retain and reuse one Equal.byReference wrapper; a fresh wrapper is a distinct member. If a public contract must accept raw object identities, moving to HashSet changes that contract; do not use Equal.byReferenceUnsafe as a general replacement. This rule reports built-in Set type references outside ambient declarations, even when a related value crosses a call boundary."

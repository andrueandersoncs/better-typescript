...
func TestRule(t *testing.T) {
	ruletest.Assert(t, "testdata", UnusedFieldRule, []analysis.Violation{
		...
		{RuleName: "unused-field", Level: "error", Message: "LocalBook.title is constructed but never independently read. Delete the speculative field or connect it to behavior that consumes its semantics.", FilePath: "src/schema-record.ts", Line: 15, Column: 48},

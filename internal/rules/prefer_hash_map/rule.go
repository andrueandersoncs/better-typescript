...
var constructorMessage = rule.RuleMessage{Id: "prefer-hash-map", Description: "Avoid constructing a built-in Map.", Help: "Use Effect's HashMap instead — for example HashMap.fromIterable([[\"a\", 1]]) or HashMap.empty(). HashMap uses Equal and Hash with structural equality by default. For reference-identity object keys, retain one Equal.byReference wrapper and use that same wrapper for every insertion and lookup. Each call creates a new wrapper, so wrapping the raw object again does not preserve native-identity lookup. If a public contract must accept raw object identities, moving to HashMap changes that contract; do not use Equal.byReferenceUnsafe as a general replacement. Constructing a Map is exempt when it is passed directly, or through a variable, to a resolved call declared outside the current source file; that includes first-party modules, not only third-party APIs."}
var typeHint = "Use HashMap.HashMap<K, V> from Effect instead. HashMap uses Equal and Hash with structural equality by default. For reference-identity object keys, retain and reuse one Equal.byReference wrapper; a fresh wrapper is a distinct key. If a public contract must accept raw object identities, moving to HashMap changes that contract; do not use Equal.byReferenceUnsafe as a general replacement. This rule reports built-in Map type references outside ambient declarations, even when a related value crosses a call boundary."

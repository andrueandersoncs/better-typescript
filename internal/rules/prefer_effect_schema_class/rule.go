...
var Rule = rule.Rule{Name: "prefer-effect-schema-class", Run: func(ctx rule.RuleContext, _ any) rule.RuleListeners {
	...
	object := func(node *ast.Node) {
		...
		ctx.ReportNode(name, rule.RuleMessage{Id: "prefer-effect-schema-class", Description: fmt.Sprintf("Avoid declaring %s as %s when this project constructs its values.", typeName, kindLabel), Help: fmt.Sprintf("Object literals of this shape are built in %s, so %s is a data model rather than a boundary type. Define it as an Effect Schema class — export class %s extends Schema.Class<%s>(%q)({ ... }) {}. Construct trusted values with %s.make({ ... }) and decode unknown input at the boundary. Use Schema.TaggedClass for tagged variants and Schema.TaggedErrorClass only for typed errors; keep process-bound runtime values as boundary types or explicit runtime data.", relative, typeName, typeName, typeName, typeName, typeName)})
	}
	typeAlias := func(node *ast.Node) {
		if tuple(node) {
			name := node.Name()
			typeName := name.AsIdentifier().Text
			ctx.ReportNode(name, rule.RuleMessage{Id: "prefer-effect-schema-class", Description: fmt.Sprintf("Avoid declaring %s as a tuple type alias.", typeName), Help: "Replace a constructed tuple alias with a named Effect Schema class with named fields, for example export class Example extends Schema.Class<Example>(\"Example\")({ myString: Schema.String, myNumber: Schema.Number }) {}. Keep a tuple only when its positions are inherently meaningful; process-bound runtime values remain boundary types or explicit runtime data."})
			return
		}

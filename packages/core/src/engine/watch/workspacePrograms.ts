import { Array, Data, Effect, Function, HashMap, MutableRef, Option, pipe } from "effect"
...
const makeWorkspacePrograms = () => {
  ...
  const createWorkspaceServices = (
    ...
    // Compiler resources stay private because only this acquisition owns their shared lifetime.
  ...
  const releaseWorkspaceServices = (
  ...
  const makeWorkspaceUpdateFrom =
    (rootPath: string) =>
    (services: ReturnType<typeof createWorkspaceServices>): WorkspaceUpdate =>
      new WorkspaceUpdate({
        rootPath,
  ...
  class WorkspacePrograms extends Data.Class<{
    readonly materialize: typeof materialize
  }> {}

  return new WorkspacePrograms({ materialize })
}

export const workspacePrograms = makeWorkspacePrograms()

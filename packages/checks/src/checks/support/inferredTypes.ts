...
const findingsInSource = (
  ...
  const checkContext = new CheckContext({ ...context, sourceFile: original })
  ...
  return pipe(
    ...
    Option.map((source) => {
      ...
      const findings = Array.filter(probes, (candidate) => {
        const hasDiagnostic = candidateHasDiagnostic(diagnostics, declarations, candidate)
        const equivalent = candidateTypesEquivalent(checker, declarations, candidate)
        const findingFlags = Array.make(!hasDiagnostic, equivalent)

        return Array.every(findingFlags, Boolean)
      })
...
export const inferredTypePlan = withProgramIndex(buildFindingIndex)((index) => {

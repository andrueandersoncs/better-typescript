...
export const usageFor = (index: ExportReferenceIndex) => (entry: ExportedFunctionEntry) => {
  const symbolKey = Equal.byReferenceUnsafe(entry.symbol)

  return pipe(HashMap.get(index.usages, symbolKey), Option.getOrElse(emptyUsage))
}

export const symbolUsageFor = (index: ExportSymbolIndex) => (entry: ExportedSymbolEntry) => {
  const symbolKey = Equal.byReferenceUnsafe(entry.symbol)

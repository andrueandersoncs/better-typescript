...
const makeTestPastInterface = (examples: NonEmptyRefactorExamples) => {
  ...
  return deriveSignals(testPastInterfaceAdvice)
}

export const testPastInterface = pipe(
  packageExamples("test-past-interface"),
  Effect.map(makeTestPastInterface)
)

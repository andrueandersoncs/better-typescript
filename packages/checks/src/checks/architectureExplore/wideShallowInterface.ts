...
const makeWideShallowInterface = (examples: NonEmptyRefactorExamples) => {
  ...
  return deriveSignals(wideShallowAdvice)
}

export const wideShallowInterface = pipe(
  packageExamples("wide-shallow-interface"),
  Effect.map(makeWideShallowInterface)
)

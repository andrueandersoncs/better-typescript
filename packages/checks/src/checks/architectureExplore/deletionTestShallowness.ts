...
const makeDeletionTestShallowness = (examples: NonEmptyRefactorExamples) => {
  ...
  return deriveSignals(deletionAdvice)
}

export const deletionTestShallowness = pipe(
  packageExamples("deletion-test-shallowness"),
  Effect.map(makeDeletionTestShallowness)
)

...
export const pipelineHostile = Effect.gen(function* () {
  const examples = yield* packageExamples("pipeline-hostile")
  ...
  const pipelineHostileAdviceFromInput = (input: PipelineHostileInput): Stream.Stream<Advice> => {
  ...
  return pipelineHostileAdviceFromInput
})

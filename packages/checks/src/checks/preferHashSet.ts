...
const isSetIdentifier = flow(Struct.get<ts.Identifier, "text">("text"), strictEqual("Set"))
...
const mutableHashSetImportMatches = (context: CheckContext) => {
  ...
  const matches = (declaration: ts.ImportDeclaration): ReadonlyArray<Detection> => {
    const isMutableHashSetModule = strictEqual(mutableHashSetModuleName)
    const isEffectModule = strictEqual(effectModuleName)

    const mutableHashSetSpecifier = (specifier: ts.ImportSpecifier) =>
      strictEqual(mutableHashSetName)(specifier.propertyName?.text ?? specifier.name.text)
...
const isMutableHashSetAccess = (access: ts.PropertyAccessExpression) =>
  strictEqual(mutableHashSetName)(access.name.text)

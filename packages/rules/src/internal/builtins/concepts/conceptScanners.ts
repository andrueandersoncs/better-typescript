...
import { conceptRuleKindSchema } from "./conceptRuleKinds.js"
...
// ConceptRuleKind stays local because the schema owns the closed set.
type ConceptRuleKind = typeof conceptRuleKindSchema.Type
...
// ConceptRuleData exists because its fields form one stable data contract used by the linter.
export const ConceptRuleData = Schema.Struct({
  kind: conceptRuleKindSchema,
...
export interface ConceptRuleData extends Schema.Schema.Type<typeof ConceptRuleData> {}
...
const conceptRuleSubscriptions = (kind: ConceptRuleKind) => (index: ConceptIndex) => {
  const entryOrder = Order.mapInput(
    Order.String,
    (entry: DataStructureEntry) => `${entry.sourceFile.fileName}:${entry.name}`
  )

  const matches = (context: MatchContext): ReadonlyArray<FactMatch<ConceptRuleData>> => {
    const found = MutableList.make<FactMatch<ConceptRuleData>>()
    ...
    const recognizesParameterBag = strictEqual("parameter-bag")(kind)
    const recognizesPassThroughConversion = strictEqual("pass-through-conversion")(kind)
    const collectionRuleChecks = Array.make(recognizesParameterBag, recognizesPassThroughConversion)
    const recognizesCollectionRule = Array.some(collectionRuleChecks, Boolean)
    const hasNoEntries = Array.isReadonlyArrayEmpty(entries)
    const recognizesEntryRule = !recognizesCollectionRule
    const skipsEmptyEntryFile = hasNoEntries && recognizesEntryRule

    if (skipsEmptyEntryFile) {
      return Array.empty()
    }
    ...
    const append = (node: ts.Node) => (data: ConceptRuleData) => {
    ...
    const emitClosedPair = (pair: readonly [DataStructureEntry, FunctionEntry]) => {
      ...
      const data = makeRuleData(owner.name)(relatedConcepts)(externalCallers)(entry)
      return append(entry.nameNode)(data)
    }

    if (strictEqual("closed-abstraction")(kind)) {
    ...
    const emitRedundantPair = (pair: readonly [DataStructureEntry, DataStructureEntry]) => {
      ...
      const data = makeRuleData(target.name)(relatedConcepts)(0)(entry)

      return append(entry.nameNode)(data)
    }

    if (strictEqual("redundant-alias")(kind)) {
    ...
    if (strictEqual("duplicate-shape")(kind)) {
      Array.forEach(duplicatePairs, emitDuplicatePair)
    }

    const recognizesClosedAbstraction = strictEqual("closed-abstraction")(kind)
    const recognizesDuplicateShape = strictEqual("duplicate-shape")(kind)
    const recognizesRedundantAlias = strictEqual("redundant-alias")(kind)

    const structuralRuleChecks = Array.make(
      recognizesClosedAbstraction,
      recognizesDuplicateShape,
      recognizesRedundantAlias
    )

    const recognizesStructuralRule = Array.some(structuralRuleChecks, Boolean)

    if (recognizesStructuralRule) {
      return MutableList.toArray(found)
    }

    if (!recognizesCollectionRule) {
      ...
      Array.forEach(entries, (entry) => {
        ...
        const emitFunctionDerived = (
          emission: readonly [FunctionEntry, HashSet.HashSet<ReferenceKey<ts.Symbol>>]
        ) => {
        ...
        const recognizesFunctionDerivedModel = strictEqual("function-derived-model")(kind)

        pipe(
          functionDerivedEmission,
          Option.filter(Function.constant(recognizesFunctionDerivedModel)),
          Option.map(emitFunctionDerived)
        )
        ...
        const sourceFileMatchesEntry = strictEqual(entry.sourceFile)
        const sourceFileIsExternal = flow(sourceFileMatchesEntry, strictEqual(false))

        const externalOwners = pipe(
          owners,
          Array.fromIterable,
          Array.filterMap(Function.flow(ownerSourceFile, Result.fromOption(Function.constVoid))),
          Array.filter(sourceFileIsExternal)
        )
        ...
        const recognizesSpeculativeExport = strictEqual("speculative-export")(kind)
        const emitSpeculativeExport = speculative && recognizesSpeculativeExport

        if (emitSpeculativeExport) {
          const relatedConcepts = Array.empty<string>()
          const data = makeRuleData("")(relatedConcepts)(0)(entry)
        ...
        const hasUnusedFields = unusedFields.length > 0
        const recognizesUnusedField = strictEqual("unused-field")(kind)
        const emitUnusedFields = hasUnusedFields && recognizesUnusedField
        ...
        const rationaleMissing = !rationaleIsComplete(entry)
        const recognizesMissingRationaleRule = strictEqual("missing-rationale")(kind)
        const recognizesMissingRationale = recognizesMissingRationaleRule && rationaleMissing

        if (recognizesMissingRationale) {
          const relatedConcepts = Array.empty<string>()
          const data = makeRuleData("")(relatedConcepts)(0)(entry)
    ...
    if (strictEqual("parameter-bag")(kind)) {
      pipe(
        index.parameterBags,
        ...
        Array.forEach((bag) => {
          ...
          const data = makeRuleData(bag.functionEntry.name)(relatedConcepts)(externalCallers)(
            bag.model
          )
    ...
    if (strictEqual("pass-through-conversion")(kind)) {
      pipe(
        ...
        Array.forEach((conversion) => {
          ...
          const data = makeRuleData(conversion.functionEntry.name)(related)(externalCallers)(
            conversion.target
          )
...
export const functionDerivedModelScanner = makeConceptScanner("function-derived-model")
export const missingRationaleScanner = makeConceptScanner("missing-rationale")
export const parameterBagScanner = makeConceptScanner("parameter-bag")
export const passThroughConversionScanner = makeConceptScanner("pass-through-conversion")
export const redundantAliasScanner = makeConceptScanner("redundant-alias")
export const speculativeExportScanner = makeConceptScanner("speculative-export")
export const unusedFieldScanner = makeConceptScanner("unused-field")

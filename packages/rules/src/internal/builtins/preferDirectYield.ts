...
// PreferDirectYieldFact exists because its fields form one stable data contract used by the linter.
...
// emptyPreferDirectYieldFact exists because its fields form one stable data contract used by the linter.
    ...
    pipe(
      Option.gen(function* () {
        ...
        const appendMatchingReference =
          (node: ts.Node) =>
          (references: ReadonlyArray<ts.Identifier>): ReadonlyArray<ts.Identifier> => {
        ...
        const appendMatchingReferenceFold = Function.untupled(
          ([references, node]: readonly [ReadonlyArray<ts.Identifier>, ts.Node]) =>
            appendMatchingReference(node)(references)
        )

        const foldReferences = foldAst(appendMatchingReferenceFold)(generator)

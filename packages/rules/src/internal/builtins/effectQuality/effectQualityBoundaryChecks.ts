...
const makeEffectQualityBoundaryChecks = () => {
  ...
  const serviceMethodFinding = makeRuleCandidate("service-method-effect-fn")
  ...
  const serviceMethodFindingForName =
    (serviceName: string) => (property: ts.ObjectLiteralElementLike) => (name: string) => {
      ...
      return serviceMethodFinding(subject)(targetNode)
    }

  const schemaClassModelNames = Array.make("Class", "TaggedClass")
  const unsafeCastFindingFromTypeNode = makeRuleCandidate("unsafe-casts")("as any")
    ...
    (node: ts.Node): ReadonlyArray<EffectQualityRuleCandidate> =>
      pipe(
        ...
        Option.map((declaration) => {
          ...
          return makeRuleCandidate("typescript-namespaces")(subject)(targetNode)
        }),
        Option.toArray
      )
  ...
  const classExtendsSchemaModel =
    (checker: ts.TypeChecker) => (declaration: ts.ClassDeclaration) => {
      const extendsSchemaMember = Function.flip(classExtendsEffectApi(checker)("Schema"))(
        declaration
      )
  ...
  const classQualityFromNode = (context: MatchContext) => (node: ts.Node) =>
    pipe(
      ...
      Option.map((declaration) => {
        ...
        return makeRuleCandidate("schema-class-models")(subject)(targetNode)
      })
    )
  ...
  const callQualityFromNode = (context: MatchContext) => (node: ts.Node) =>
    pipe(
      ...
      Option.map((call) => {
        ...
        return makeRuleCandidate("schema-class-models")(subject)(call.expression)
      })
    )
  ...
  const schemaRecordInterfaceFindingFromDeclaration = (declaration: ts.VariableDeclaration) =>
    makeRuleCandidate("schema-record-interface")((declaration.name as ts.Identifier).text)(
  ...
  const schemaOptionalKeyFindingFromCall = (fieldName: string) => (call: ts.CallExpression) =>
    makeRuleCandidate("schema-optional-key")(fieldName)(call.expression)
    ...
    (node: ts.Node): ReadonlyArray<EffectQualityRuleCandidate> =>
      pipe(
        ...
        Option.map((declaration) => {
          ...
          return makeRuleCandidate("schema-error-class")(subject)(targetNode)
        }),
        Option.toArray
      )
  ...
  const serviceMethodFindingFromProperty =
    (context: MatchContext) => (serviceName: string) => (property: ts.ObjectLiteralElementLike) =>
      pipe(
        ...
        EffectMatch.orElse(Option.none as () => Option.Option<EffectQualityRuleCandidate>),
  ...
  const serviceMethodAssignmentFindings =
    (context: MatchContext) =>
    (serviceName: string) =>
    (object: ts.ObjectLiteralExpression): ReadonlyArray<EffectQualityRuleCandidate> =>
    ...
    (node: ts.Node): ReadonlyArray<EffectQualityRuleCandidate> =>
      pipe(
        ...
        Option.map((inspection) => {
          ...
          return makeRuleCandidate("effect-fn-name")(subject)(inspection.node as ts.Node)
        }),
        Option.toArray
      )
  ...
  const rawFetchFinding = makeRuleCandidate("raw-fetch-abort-signal")
  ...
  const uncurriedContainsFetch = Function.untupled(
    ([found, current]: readonly [boolean, ts.Node]) => containsFetch(found)(current)
  )
  ...
  const callbackPassesSignalToFetch = (signalName: string) => {
    ...
    const uncurriedPassesSignal = Function.untupled(
      ([found, current]: readonly [boolean, ts.Node]) => passesSignal(found)(current)
    )
  ...
  const responseValidationFinding = makeRuleCandidate("http-response-validation")
  ...
  const httpResponseValidationFindings =
    (context: MatchContext) => (_index: EffectQualityIndex) => (node: ts.Node) => {
  ...
  const statusDecodeOrderFinding = makeRuleCandidate("http-status-decode-order")
  ...
  const testStyleFinding = makeRuleCandidate("effect-test-style")
  ...
  const findingWhen =
    (shouldEmit: boolean) =>
    (finding: EffectQualityRuleCandidate): ReadonlyArray<EffectQualityRuleCandidate> =>
      shouldEmit ? Array.of(finding) : emptyRuleCandidates
  ...
  const configRefinedValues =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const finding = makeRuleCandidate("config-refined-values")(subject)(node.expression)
  ...
  const retryWithoutJitter =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const finding = makeRuleCandidate("retry-without-jitter")(subject)(node.expression)
  ...
  const idempotentRetry =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const alreadyIdempotent = index.isIdempotentOperationName(operation)
      ...
      const finding = makeRuleCandidate("idempotent-retry")(subject)(node.expression)
  ...
  const rawFetchOutsideAdapter =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const finding = makeRuleCandidate("raw-fetch-outside-adapter")("fetch")(node.expression)
  ...
  const httpClientPreference =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const shouldIgnoreHttpClientPreference = Array.some(
        ignoreHttpClientPreferenceReasons,
        Boolean
      )
      ...
      const finding = makeRuleCandidate("http-client-preference")("fetch")(node.expression)
  ...
  const boundarySchemaDecode =
    ...
    (node: ts.CallExpression): ReadonlyArray<EffectQualityRuleCandidate> => {
      ...
      const finding = makeRuleCandidate("boundary-schema-decode")(subject)(node.expression)
  ...
  const callCandidates =
    ...
    (context: MatchContext) =>
    (index: EffectQualityIndex) =>
      flow((node: ts.Node) => node as ts.CallExpression, find(context)(index))
  ...
  const schemaCheck = makeCheck(schemaKinds)
  const callCheck = makeCheck(callKinds)
  const callCheckWithoutIndex = flow(candidatesWithoutIndex, callCheck)
  const callExpressionCheckWithoutIndex = flow(callCandidatesWithoutIndex, callCheck)
  const unsafeCast = schemaCheck(unsafeCastFindings)("unsafe-casts")
  const schemaClass = schemaCheck(schemaClassModelFindings)("schema-class-models")
  const namespaces = schemaCheck(typescriptNamespaceFindings)("typescript-namespaces")
  const serviceEffectFn = schemaCheck(serviceMethodEffectFnFindings)("service-method-effect-fn")
  const effectFnName = schemaCheck(effectFnNameFindings)("effect-fn-name")
  const schemaRecord = schemaCheck(schemaRecordInterfaceFindings)("schema-record-interface")
  const optionalKey = schemaCheck(schemaOptionalKeyFindings)("schema-optional-key")
  const schemaError = schemaCheck(schemaErrorClassFindings)("schema-error-class")
  const fetchAbort = callCheckWithoutIndex(rawFetchAbortFindings)("raw-fetch-abort-signal")
  const responseValidation = callCheck(httpResponseValidationFindings)("http-response-validation")
  const statusDecodeOrder = callCheck(httpStatusDecodeOrderFindings)("http-status-decode-order")
  const testStyle = callCheck(effectTestStyleFindings)("effect-test-style")

  const configRefinement =
    callExpressionCheckWithoutIndex(configRefinedValues)("config-refined-values")

  const jitter = callExpressionCheckWithoutIndex(retryWithoutJitter)("retry-without-jitter")
  const rawFetchCandidates = callCandidates(rawFetchOutsideAdapter)
  const rawFetch = callCheck(rawFetchCandidates)("raw-fetch-outside-adapter")
  const httpCandidates = callCandidates(httpClientPreference)
  const httpClient = callCheck(httpCandidates)("http-client-preference")

  const boundaryDecode =
    callExpressionCheckWithoutIndex(boundarySchemaDecode)("boundary-schema-decode")

  const retryCandidates = callCandidates(idempotentRetry)
  const idempotent = callCheck(retryCandidates)("idempotent-retry")
...
export const effectQualityBoundaryChecks = makeEffectQualityBoundaryChecks()

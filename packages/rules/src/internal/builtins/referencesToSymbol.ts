...
export const referencesToSymbol =
  (checker: ts.TypeChecker) => (symbol: ts.Symbol) => (root: ts.Node) => {
    const symbolAt = symbolOptionAt(checker)

    const countMatchingIdentifier = (node: ts.Node) => (count: number) => {
    ...
    const countMatchingIdentifierFold = Function.untupled(
      ([count, node]: readonly [number, ts.Node]) => countMatchingIdentifier(node)(count)
    )

    return foldAst(countMatchingIdentifierFold)(root)(0)
  }

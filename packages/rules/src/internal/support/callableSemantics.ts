...
const makeRelationEntry = (index: number) => (word: string) => {
...
const firstRelation = (words: ReadonlyArray<string>) =>
  pipe(
    ...
    Array.filterMap((word, index) =>
      pipe(makeRelationEntry(index)(word), Result.fromOption(Function.constVoid))
    ),
    Array.head
  )
...
const ownerName = (scan: FunctionDefinition) =>
  pipe(
    EffectMatch.value(scan.parent),
...
const functionName = (scan: FunctionDefinition) => {
  const directName = pipe(Option.fromNullishOr(scan.name), Option.flatMap(identifierName))
  const enclosingName = ownerName(scan)
...
const parameterSymbols =
  ...
  (scan: FunctionDefinition): ReadonlyArray<ts.Symbol> => {
    ...
    return Array.filterMap(scan.parameters, parameterSymbol)
  }
  ...
  (returnType: ts.Type): ts.Type => {
    ...
    const visit =
      ...
      (current: ts.Type): ts.Type => {
        ...
        return complete ? current : visit(remainingDepth - 1)(nextType)
      }

    return visit(4)(returnType)
  }
...
const callableResult = (checker: ts.TypeChecker) => (scan: FunctionDefinition) =>
  pipe(
...
const projectionEvidence =
  (checker: ts.TypeChecker) =>
  (scan: FunctionDefinition): Option.Option<ProjectionEvidence> => {
    const symbolsFor = parameterSymbols(checker)
    const scans = callableDefinitions(scan)
    const terminal = terminalDefinition(scan)
    const bindings = Array.flatMap(scans, symbolsFor)
...
const appendExpressionOperationWordsFold = Function.untupled(
  ([words, node]: readonly [ReadonlyArray<string>, ts.Node]) =>
    appendExpressionOperationWords(node)(words)
)

const foldExpressionOperationWords = foldAst(appendExpressionOperationWordsFold)

const expressionOperationWords = Function.flip(foldExpressionOperationWords)(emptyStrings)

const returnedOperationWords = (scan: FunctionDefinition): ReadonlyArray<string> =>
  pipe(resultExpressions(scan), Array.flatMap(expressionOperationWords), Array.dedupe)
...
const makeSemanticRoleEntry = (role: SemanticRole) => (present: boolean) =>
  Tuple.make(role, present)
...
const buildCallableSemantics = (checker: ts.TypeChecker) => (scan: FunctionDefinition) =>
  Option.gen(function* () {
    const nameNode = yield* functionName(scan)
    const terminal = terminalDefinition(scan)
    const result = yield* callableResult(checker)(terminal)
    const name = makeCallableNameClaims(nameNode)
    const projection = projectionEvidence(checker)(scan)
    const expressions = resultExpressions(terminal)
    ...
    const scanSourceWords = sourceWords(checker)(scan)
    return new CallableSemantics({
      scan,
      node: nameNode,
      name,
      result: enrichedResult,
      sourceWords: scanSourceWords,
...
const programTypeChecker = (program: ts.Program) => program.getTypeChecker()

const callableSemanticsForProgram = Function.memoize(
  flow(programTypeChecker, buildCallableSemantics, Function.memoize)
)

export const callableSemantics = flow(
  Struct.get<ProgramContext, "program">("program"),
  callableSemanticsForProgram
)

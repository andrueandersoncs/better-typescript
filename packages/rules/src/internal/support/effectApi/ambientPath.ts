...
export const ambientPathAt =
  ...
  (expression: ts.Expression): Option.Option<ReadonlyArray<string>> => {
    const pathRootIsAmbient = flow(
      Tuple.get<readonly [ts.Identifier, ReadonlyArray<string>], 0>(0),
      symbolIsAmbient(checker)
    )

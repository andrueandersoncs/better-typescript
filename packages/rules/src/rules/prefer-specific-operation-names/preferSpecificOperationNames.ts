import {
  ...
  lookupOperations
} from "../../internal/support/operationVocabulary.js"
import {
  Array,
  Function,
  HashMap,
...
const matches = (context: MatchContext) => {
  ...
  const matchFunctionDefinition = (scan: FunctionDefinition) =>
    pipe(semanticsFor(scan), Option.flatMap(matchFromSemantics), Option.toArray)

  return matchFunctionDefinition
}

export const preferSpecificOperationNamesScanner = functionDefinitionScanner(matches)

...
const typeText = (checker: ts.TypeChecker) => (node: ts.Node) => (type: ts.Type) =>
...
export const typesEquivalent =
  ...
  (right: ts.Type) => {
    ...
    const leftText = typeText(checker)(leftNode)(left)
    const rightText = typeText(checker)(rightNode)(right)

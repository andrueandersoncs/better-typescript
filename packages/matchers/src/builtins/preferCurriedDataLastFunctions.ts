...
const foldCurriedDataLastDescendants = <A>(visit: (node: ts.Node) => (accumulator: A) => A) => {
  const foldVisit = (current: A, currentNode: ts.Node) => visit(currentNode)(current)

  return foldAst(foldVisit)
}
...
export const preferCurriedDataLastFunctionsMatcher = makeMatcherFromSubscriptions(

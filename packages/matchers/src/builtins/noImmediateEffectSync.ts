...
const initializerIsEffectSync = (context: MatchContext, declaration: ts.VariableDeclaration) => {
  const isEffectSyncCall = (call: ts.CallExpression) =>
    importedEffectApiAt(context.checker, call.expression, "Effect", syncNames)
...
const matchingSyncDeclaration = (
  ...
  Option.gen(function* () {
    ...
    const isEffectSyncDeclaration = (candidate: ts.VariableDeclaration) =>
      initializerIsEffectSync(context, candidate)

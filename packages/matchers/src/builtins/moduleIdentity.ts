...
const makeModuleIdentity = () => {
  class AliasIndex extends Data.Class<{
    readonly aliasesByFile: HashMap.HashMap<string, ReadonlyArray<string>>
  }> {}

  const unknownRecordSchema = Schema.Record(Schema.String, Schema.Unknown)
  const decodeUnknownRecord = Schema.decodeUnknownOption(unknownRecordSchema)

  const packageExportsJson = pipe(
    Schema.Struct({
  ...
  const moduleIdentityElements =
    (index: AliasIndex) =>
    (context: MatchContext): ReadonlyArray<MatcherMatch<ModuleIdentityData>> =>
      pipe(
  ...
  return programIndexedFileMatcher(buildAliasIndex)(moduleIdentityElements)
}

export const moduleIdentity = makeModuleIdentity()

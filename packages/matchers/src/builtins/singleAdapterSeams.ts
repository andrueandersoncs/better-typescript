...
const makeSingleAdapterSeams = () => {
  ...
  const buildIndex = (
    ...
    const scanFile =
      ...
      (
        ...
        const isCandidateSymbol = (symbol: ts.Symbol) => {
          const symbolKey = referenceKey(symbol)
        ...
        const foldNode = (
          ...
          const parameterState = pipe(
            ...
            Option.filter(isCandidateSymbol),
            Option.map((candidate) => {
          ...
          const classState = pipe(
            ...
            Option.map((declaration) => {
              const symbols = pipe(
                ...
                Array.filter(isCandidateSymbol)
              )
        ...
        return foldAst(foldNode)(sourceFile)(state)
      }
  ...
  const singleAdapterElements =
    ...
    (context: MatchContext): ReadonlyArray<Match<SingleAdapterSeamData>> => {
  ...
  return programIndexedFileMatcher(buildIndex)(singleAdapterElements)
}

export const singleAdapterSeams = makeSingleAdapterSeams()

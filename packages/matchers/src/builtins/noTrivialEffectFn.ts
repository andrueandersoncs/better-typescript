...
const callProvidesEffectFnServiceObject =
  ...
  (
    ...
    const isLayerSucceed = importedEffectApiAt(
      context.checker,
      callee,
      "Layer",
      effectFnLayerSucceedNames
    )
    ...
    const expressionIsService = (expression: ts.Expression) =>
      isContextServiceAt(context, expression)
...
const effectFnServiceOfCall = (context: MatchContext) => (call: ts.CallExpression) => {
  ...
  const accessReceiverIsService = (access: ts.PropertyAccessExpression) =>
    isContextServiceAt(context, access.expression)
...
const objectIsEffectFnServiceImplementation =
  (context: MatchContext) => (object: ts.ObjectLiteralExpression) => {
    ...
    const directCallProvidesObject = (call: ts.CallExpression) =>
      callProvidesEffectFnServiceObject(context, object)(call, call, 0, 1)
    ...
    const isCurriedLayerImplementation = pipe(
      Option.gen(function* () {
        ...
        return callProvidesEffectFnServiceObject(context, object)(serviceCall, objectCall, 0, 0)
      }),
  ...
  (context: MatchContext) => (declaration: ts.VariableDeclaration) => {
    ...
    const referencedAsServiceProperty = pipe(
      ...
      Option.exists((symbol) => {
        const matchesServiceProperty = (found: boolean, node: ts.Node) => {
        ...
        return foldAst(matchesServiceProperty)(context.sourceFile)(false)
      })
    )

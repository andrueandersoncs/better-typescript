import { Array, Function, Match, Option, Record, Struct, flow, pipe } from "effect"
...
const stateConstructorEntries = Record.toEntries(stateConstructors)
...
export const adapterBoundaryDetections = (
  ...
  const hasScopedLifecycleAncestor = (checker: ts.TypeChecker, currentNode: ts.Node) => {
    const scopedEffect = hasEffectCallAncestor(checker, currentNode, "Effect", effectLifecycleNames)
  ...
  const detectionsForKind = (
    ...
    const detect = (subject: string) => boundaryDetection(context, subjectNode, role, kind, subject)

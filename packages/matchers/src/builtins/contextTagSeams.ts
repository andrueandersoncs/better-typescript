...
const makeContextTagSeams = () => {
  ...
  const buildIndex = (
    ...
    const scanFile =
      ...
      (
        ...
        type SeamCounts = HashMap.HashMap<
          ReferenceKey<ts.Symbol>,
          readonly [number, number, number]
        >

        const foldNode = (current: SeamCounts, node: ts.Node): SeamCounts => {
          const resolveIdentifierCounts = (identifier: ts.Identifier) => {
          ...
          return pipe(
            ...
            Option.flatMap(resolveIdentifierCounts),
            Option.getOrElse(Function.constant(current))
          )
        }

        return foldAst(foldNode)(sourceFile)(counts)
      }
  ...
  const contextTagSeamElements =
    ...
    (context: MatchContext): ReadonlyArray<Match<ContextTagSeamData>> => {
  ...
  return programIndexedFileMatcher(buildIndex)(contextTagSeamElements)
}

export const contextTagSeams = makeContextTagSeams()

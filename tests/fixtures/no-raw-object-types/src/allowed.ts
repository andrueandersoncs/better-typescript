...
// Array of named type
export const countAddresses = (addresses: ReadonlyArray<ServerAddress>): number =>
  addresses.length
...
export const firstAddress = (addresses: Map<string, ServerAddress>): ServerAddress | undefined =>
  addresses.values().next().value

// No type annotation on parameter (inferred)
export const identity = (value: number) => value

// Union of named types
export const stringOrNumber = (value: string | number): string =>
  String(value)
...
export const applyFn = (transform: (value: number) => number, input: number): number =>
  transform(input)
...
export const greetUser = (user: UserRecord): string =>
  `Hello, ${user.name}`

// Tuple type — not an anonymous object
export const swapTuple = (pair: [string, number]): [number, string] => [pair[1], pair[0]]

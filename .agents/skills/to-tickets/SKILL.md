...
The issue tracker and triage label vocabulary should have been provided to you. If not, tell the
...
- **Local files** → write one file per ticket under `.scratch/<feature-slug>/issues/<NN>-<slug>.md`,
  ...
  numbers/titles it depends on. Use the per-ticket file template below: one ticket per file, never a
...
**What to build:** the end-to-end behaviour this ticket makes work, from the user's perspective, not

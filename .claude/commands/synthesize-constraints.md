...
- 7. **Audit terminology and example coverage before writing the document.** Re-read the planned definitions and constraints as a reader without repository context. Add a definition for every technical term, category, classification, action, state, boundary, or relationship that remains undefined; add or correct every required inline definition link; and remove no definitions merely to keep the glossary short. For every definition, make a checklist of every enumerated item in its prose—including observable inputs, membership requirements, alternatives, categories, facilities, and exclusions—and verify that the immediately following example set demonstrates each item with the required adjacent label. Revise the definition or example set until that checklist is complete.

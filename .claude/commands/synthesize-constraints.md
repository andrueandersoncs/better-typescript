...
- A semantic classification, including a module role, MUST be inferred exclusively from the classified module's source AST, compiler-resolved symbols and types, and resolved dependency graph.
- It MUST NOT use a path, directory, basename, file extension, package layout, declaration or export name, user mapping, or presumed intent.
- A physical-file rule MAY use path or filename criteria, but MUST remain independent of the semantic classification.
- When a later constraint references a module role, its verifier MUST apply this content-derived predicate. It MUST NOT rederive the role from a physical-file convention.
...
- Every classification used by a definition or constraint MUST be inferred deterministically from the artifact it classifies: source text or AST, compiler-resolved symbols or types, the resolved module graph, filename or directory conventions, or existing project configuration that defines compiler or build inputs.
- A semantic classification, including a module role, is stricter: it MUST use only source AST, compiler-resolved symbols or types, and the resolved module graph.
...
- Derive rules for every applicable physical and public-interface choice: repository-root and directory placement, file basename and suffix, module specifier form, permitted imports and dependency direction, content-derived module role, declaration order, permitted top-level contents, and permitted exports.
- Physical conventions MAY constrain location or spelling, but MUST NOT assign or participate in assignment of a module role.
- State exact deterministic path or filename criteria for physical-file rules and exact AST, graph, or compiler-resolution criteria for semantic rules.
- A module role MUST be derived from its source contents alone, never from its path, extension, directory, package layout, or declaration/export names.
...
- Repeat the audit specifically for structural modularity: construct counterexamples involving misplaced files, impermissible filenames or suffixes, invalid module specifiers, module roles that conflict with source contents, reversed dependency direction, disallowed module contents, declaration ordering, and excessive or misplaced exports whenever those choices can affect the requested quality.

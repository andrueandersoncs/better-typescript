...
```
Each definition MUST be a separate `### <Term>` entry. Use the entry heading as the link target for every subsequent prose use of that term. The `## Definitions` section has no maximum number of entries.

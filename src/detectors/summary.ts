...
export class AdviceElement extends Schema.Class<AdviceElement>("AdviceElement")(
  {
    location: Location,
...
// An advice derivation folds completed upstream signals into emissions of its own.
...
export class CountSummary extends Schema.Class<CountSummary>("CountSummary")({
  ...
  countsByRule: Schema.Any,
  filesByRule: Schema.Any
}) {
  declare readonly countsByRule: HashMap.HashMap<string, number>
  declare readonly filesByRule: HashMap.HashMap<string, number>
}
...
export const countSummary = (
  ...
  const countsByRule = Array.reduce(elements, emptyCounts, addDetectionCount)
  const filesByRule = Array.reduce(files, emptyCounts, addFileRuleCounts)

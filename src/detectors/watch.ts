...
/**
 * Full rule recompute per batch: detection sets are always exactly what the
 * snapshot report would compute for the current programs. Per-file incremental
 * recompute is deliberately not built — checker-using rules observe other
...
export const signalUpdates =
  ...
  ): Stream.Stream<SignalsBatch, Error> =>
    pipe(
      updates,
...
const ruleSnapshotEquals = (a: RuleSnapshot, b: RuleSnapshot): boolean => {
...
const ruleSnapshotsEquivalence = Array.getEquivalence(ruleSnapshotEquals)

/**
 * Detection-set equality per rule. Best-effort on Detection.data by design:
 * it is Schema.Unknown, so a rule that builds a fresh plain data object per
 * run compares unequal and the batch passes through — the gate under-cuts,
 * never over-cuts, and correctness never depends on it.
 */
export const signalsEquivalence = (
  a: SignalsBatch,
  b: SignalsBatch
): boolean => {
  const sameRules = ruleSnapshotsEquivalence(a.rules, b.rules)
  const sameHelpers = ruleSnapshotsEquivalence(a.helpers, b.helpers)

  return sameRules && sameHelpers
}

/**
 * Within each element the existing advice graph runs unchanged: rule
 * snapshots are lifted to replayable RuleSignals streams and wiring.advice
 * consumes the streams it needs.
 */
export const adviceUpdates =
  (wiring: ReportWiring) =>
  (
    signals: Stream.Stream<SignalsBatch, Error>
 ...
 * projection).
 *
 * Fan-in never tears because it happens inside an element (advice reads every
 * rule signal from one SignalsBatch), never across independently-ticking
 * streams. Per-node external subscribers are derived views — broadcast the
 * SignalsBatch stream, Stream.map the projection, gate with

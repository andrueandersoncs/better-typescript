...
Better TypeScript is a Go binary with direct `typescript-go` rules under `internal/rules/`. Run it from the directory containing `tsconfig.json`. No flags checks all project files with all rules unless `better-typescript.json` contains ordered per-file `inclusion` or `exclusion` overrides. `--files` accepts project-relative globs. `--rules` accepts rule names and ignores the configuration. Both flags are repeatable or comma-separated. The CLI emits deterministic six-field NDJSON at `error` level and exits successfully after completed analysis.

...
Better TypeScript is a Go binary with direct `typescript-go` rules under `internal/rules/`. Run it from the directory containing `tsconfig.json`. All rules are on by default. Ordered `better-typescript.json` commands use matching `add_exclusions` to turn named rules off and matching `add_inclusions` to turn them back on. Commands default to deterministic rules; `"mode": "semantic"` applies the same selection without removing repository evidence. `--files` accepts project-relative globs. `--rules` accepts rule names and skips configured commands for the invoked mode. Both flags are repeatable or comma-separated. The CLI emits deterministic six-field NDJSON at `error` level, exits `1` when error-level diagnostics are emitted, and exits `0` after completed analysis without errors.

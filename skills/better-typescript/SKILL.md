...
3. For a fix request, apply the smallest behavior-preserving fixes. For `related-kind-files` relationship reports, use the reported type evidence and repository vocabulary to choose a domain module name; do not derive a directory name mechanically. Plain types only need the canonical `types.ts` filename and keep their domain owner.

...
Run `npx better-typescript semantic --range 'origin/main...HEAD'` for complete changed files from a committed range endpoint, `--files 'src/**/*.ts'` for selected current files, or `--all` for every eligible current file. Add `--rules function-naming,readonly` to select policies. `--range`, `--files`, and `--all` are mutually exclusive. Range mode uses file contents and `better-typescript.json` from the end commit. Frontmatter globs and ordered semantic configuration select each file's candidate policies. Each candidate file-policy pair declares one whole-file scope when it fits or every required overlapping window scope otherwise; candidate selection does not prove that the policy applies to the code shape, and each question requires a negative answer when the subject is absent or incompatible. At most eight request partitions run concurrently. The highest window probability becomes the policy result. The policy file is inserted verbatim into every question. Requests use `jev-latest` unless `--model` overrides it. Use `--dry-run` to inspect files, candidate policies, windows, request partitions, and encoded bytes without an API call.

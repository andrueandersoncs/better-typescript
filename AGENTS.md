...
Always read the rule modules in `src/rules/` (each module's emitted message/hint states the rule) before writing TypeScript; verify with a bounded run (`timeout 10 npm run dev`) and keep the initial report at `No signals`.
Always run the linter (`timeout 10 npm run dev`) on the codebase itself after making any changes
